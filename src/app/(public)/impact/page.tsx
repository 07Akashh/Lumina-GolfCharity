      ...
      <main className="pt-32 pb-24">
        <section className="px-8 max-w-7xl mx-auto mb-20">
          <div className="max-w-3xl space-y-6">
            <h1 className="display-xl text-[#0a1628]">Our collective <span className="serif-i text-[#c81e51]">impact.</span></h1>
            <p className="text-lg text-[#3d4151] leading-relaxed">
              Lumina is built on the principle of distributed philanthropy. Every contribution, every draw, and every winner fuels a global network of verified foundations.
            </p>
          </div>
        </section>

        {/* Dynamic Stats Grid */}
        <section className="px-8 max-w-7xl mx-auto mb-20 grid md:grid-cols-4 gap-6">
          <ImpactMetric 
        ...
        <section className="bg-[#f4f3f1] py-24">
          <div className="px-8 max-w-7xl mx-auto space-y-12">
            <div className="text-center space-y-4">
              ...
              <p className="text-[#8a8f9e] max-w-lg mx-auto">We partner with verified, high-impact organisations across environmental, social, and cultural sectors.</p>
            </div>
            
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {charities?.map(ch => (
                <div key={ch.id} className="card-lumina p-8 text-center space-y-4 hover:translate-y-1 transition-transform">
                  <div className="w-16 h-16 rounded-2xl bg-white border border-[#eceae7] flex items-center justify-center mx-auto shadow-sm">
                    {ch.image_url ? (
                      <img src={ch.image_url} alt={ch.name} className="w-10 h-10 object-contain" />
                    ) : (
                      <Heart size={24} className="text-[#c81e51]/20" />
                    )}
                  </div>
                  <h3 className="font-bold text-[#0a1628]">{ch.name}</h3>
                  <p className="text-[11px] text-[#8a8f9e] line-clamp-2">{ch.description}</p>
                </div>
              ))}
            </div>
          </div>
...
function ImpactMetric({ label, value, icon, description }: { label: string; value: string; icon: React.ReactNode; description: string }) {
  return (
    <div className="card-lumina p-8 space-y-4 border-l-4 border-[#c81e51]/10">
      ...
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.1em] text-[#8a8f9e]">{label}</p>
        <p className="text-3xl font-black text-[#0a1628] mt-1">{value}</p>
      </div>
      <p className="text-[11px] text-[#3d4151] leading-relaxed opacity-60">{description}</p>
    </div>
  )
}
