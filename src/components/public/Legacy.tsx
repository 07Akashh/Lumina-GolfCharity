...
async function getLegacyData() {
  ...
  const totalWon =
    winners?.reduce((acc, w) => acc + Number(w.prize_amount), 0) || 0;
...
export default function Legacy() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: getLegacyData,
  })

  useEffect(() => {
    setIsLoading(isLoading, 'Retrieving historical ledger...')
    ...
    <main className="pt-32 pb-24 px-8 max-w-7xl mx-auto">
      <header className="mb-24 flex flex-col md:flex-row md:items-end justify-between gap-12">
        <div className="max-w-2xl space-y-6">
          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[#c81e51]">
          ...
          <p className="text-lg text-[#3d4151] leading-relaxed">
            Every draw cycle creates a new set of winners and a fresh wave of
            philanthropic impact. Our ledger records the history of luck meeting
            generosity.
          </p>
        </div>
        <div className="card-dark p-10 text-white flex flex-col justify-end space-y-4 md:min-w-[400px]">
          ...
          <p className="display-md text-[#c81e51] font-black serif-i">
            {data ? formatCurrency(data.totalWon) : '...'}
          </p>
          <p className="text-[11px] opacity-40">
            Verified prize settlements across 12 countries.
          </p>
        </div>
      </header>
        ...
        <div className="grid md:grid-cols-2 gap-8 min-h-[400px]">
          {data ? (
            data.draws?.map((draw) => (
              <div
                key={draw.id}
                className="card-lumina p-10 group hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)] transition-all"
              >
                <div className="flex justify-between items-start mb-8">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black uppercase tracking-[0.1em] text-[#8a8f9e]">
                      {String(draw.published_at).split("T")[0]}
                    </p>
                    <h3 className="text-xl font-bold text-[#0a1628]">
                      Cycle #{draw.id.slice(0, 8)}
                    </h3>
                  </div>
                ...
                <div className="flex gap-3 mb-10">
                  {Array.isArray(draw.numbers) && (draw.numbers as number[]).map((num: number, i: number) => (
                    <div
                      key={i}
                      className="draw-ball w-12 h-12 text-sm font-bold border-[#eceae7]"
                    >
                      {num}
                    </div>
                  ))}
                ...
                <div className="flex items-end justify-between pt-8 border-t border-[#f4f3f1]">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.1em] text-[#8a8f9e]">
                      Prize Pool Delivered
                    </p>
                    <p className="text-2xl font-black text-[#0a1628] mt-1">
                      {formatCurrency(draw.prize_pool_total)}
                    </p>
                  </div>
            ...
            [1, 2, 3, 4].map((i) => (
              <div key={i} className="card-lumina p-10 animate-pulse bg-[#f4f3f1] h-[300px]" />
            ))
          )}
        </div>
      </section>
      ...
      <section className="mt-32 card-lumina bg-[#faf9f7] border-2 border-[#f4f3f1] p-16 text-center space-y-8 overflow-hidden relative">
        <div className="max-w-2xl mx-auto space-y-4 relative z-10">
          ...
          <p className="text-lg text-[#3d4151] opacity-70">
            Participation in Lumina is more than a game. It is a commitment to
            collective philanthropy.
          </p>
          <div className="pt-6 flex justify-center gap-4">
            <a
              href="/memberships"
              className="btn-primary px-10 py-4 text-sm font-bold shadow-xl shadow-[#c81e51]/20"
            >
              Establish Legacy
            </a>
            <a
              href="/impact"
              className="btn-secondary px-10 py-4 text-sm font-bold"
            >
              Explore Impact
            </a>
          </div>
